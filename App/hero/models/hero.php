...
class Hero {
     ...
     */
    public function create($title, $subtitle, $pageName, $hasCarousel) {
        try {
            $sql = "INSERT INTO heroes (hero_title, hero_subtitle, page_name, has_sliders) 
                    VALUES (:title, :subtitle, :page, :has_carousel)";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([
                ':title' => $title,
                ':subtitle' => $subtitle,
     ...
     */
    public function update($id, $title, $subtitle, $pageName, $hasCarousel) {
        try {
